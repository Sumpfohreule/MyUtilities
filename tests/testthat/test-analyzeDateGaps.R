...
test_that("Works with dplyr::group_map", {
  calculated <- data.frame(
    ...
    group_map(~ .x %>%
      pull(dates) %>%
      analyzeDateGaps(interval = 60 * 60))
  reference <- list(
    data.frame(
      gap_start = lubridate::ymd_hms("2020-01-01 00:00:00"),
...
test_that("Analyze gaps between latest given date and the end of the year", {
  dates <- lubridate::ymd(c(
    "2020-01-01",
    "2020-01-02",
    "2020-01-03"
  ))
  reference <- data.frame(
    gap_start = lubridate::as_datetime("2020-01-03"),
    gap_end = lubridate::as_datetime("2020-12-31"),
    gap_size_percent = 99
  )
  expect_equal(
    analyzeDateGaps(dates, interval = 60 * 60 * 24, extend.to.full.year = TRUE),
    reference
  )
})
...
test_that("Analyze gaps between latest given date and the start of the year", {
  ...
  reference <- data.frame(
    gap_start = lubridate::as_datetime("2020-01-01"),
