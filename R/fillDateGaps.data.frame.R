...
#'
#' For each grouping column variation defined by group_columns the specific date intervall is calculated and missing dates are filled
#' into the data.frame
...
#' @param window If intervals can change within their groups use a window ("year", "month", "day")
...
fillDateGaps.data.frame <- function(data, group_columns = NULL, date_column = NULL, window = NULL) {
    ...
    if (length(date_column) < 1) {
        stop("No date column found. Provide a date_column or convert the column to POSIXct")
    ...
    if (is.null(group_columns)) {
        ...
        new_dates <- data[, .(dates = fillDateGaps(get(date_column))), by = c(group_columns)]
    }
    setnames(new_dates,
             old = "dates",
             new = date_column)
